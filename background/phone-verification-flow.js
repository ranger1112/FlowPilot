(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    ...
    // ---- 错误识别 / 构造器拆到 background/phone-error-classifier.js ----
    // 优先用注入的 module；测试或独立加载时也可以从 self 上拿。
    const phoneErrorClassifierFactory = (phoneErrorClassifier && typeof phoneErrorClassifier.createPhoneErrorClassifier === 'function')
      ? phoneErrorClassifier.createPhoneErrorClassifier
      : (typeof self !== 'undefined' && self.MultiPageBackgroundPhoneErrorClassifier
          ? self.MultiPageBackgroundPhoneErrorClassifier.createPhoneErrorClassifier
          : null);
    if (typeof phoneErrorClassifierFactory !== 'function') {
      throw new Error('createPhoneVerificationHelpers requires phoneErrorClassifier (or globally registered MultiPageBackgroundPhoneErrorClassifier).');
    }
    const phoneErrorClassifierInstance = phoneErrorClassifierFactory({
      prefixes: {
        timeout: PHONE_CODE_TIMEOUT_ERROR_PREFIX,
        restartStep7: PHONE_RESTART_STEP7_ERROR_PREFIX,
        resendThrottled: PHONE_RESEND_THROTTLED_ERROR_PREFIX,
        resendBannedNumber: PHONE_RESEND_BANNED_NUMBER_ERROR_PREFIX,
        resendServer: PHONE_RESEND_SERVER_ERROR_PREFIX,
        route405: PHONE_ROUTE_405_RECOVERY_FAILED_ERROR_PREFIX,
        maxUsage: 'PHONE_MAX_USAGE_EXCEEDED::',
        staleSignupEmailVerificationCode: PHONE_STALE_SIGNUP_EMAIL_VERIFICATION_ERROR_CODE,
      },
      // 这两个回调依赖工厂内部 hoisted 的 function declaration，
      // 调用时才会真正访问，安全。
      formatStep9Reason: (reason) => formatStep9Reason(reason),
      normalizePhoneSmsProvider: (value) => normalizePhoneSmsProvider(value),
      phoneSmsProvider5simId: PHONE_SMS_PROVIDER_5SIM,
    });
    const {
